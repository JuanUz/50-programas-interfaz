...
# =========================================
# Programa: Ordenamiento por mezcla (simple)
# Autor: Garcia Ornelas Juan Carlos
# Descripción: Ordenamiento Mezcla
# Codigo en python y Ensamblador
# =========================================

def merge_sort(array):
    n = len(array)
    size = 1  # Tamaño inicial del bloque a mezclar

    while size < n:
        # Mezclar bloques de tamaño 'size'
        for start in range(0, n, 2 * size):
            mid = min(start + size, n)  # Punto medio del primer bloque
            end = min(start + 2 * size, n)  # Fin del segundo bloque

            # Llamada a la función de mezcla para ordenar y combinar
            merge(array, start, mid, end)

        # Duplicar el tamaño del bloque a mezclar en cada iteración
        size *= 2

    return array

def merge(array, start, mid, end):
    # Crear copias de los subarrays a mezclar
    left = array[start:mid]
    right = array[mid:end]

    # Índices para iterar por los subarrays
    i, j = 0, 0
    k = start

    # Combinar los elementos de los subarrays en el array original
...
print("Ordenando el array usando el método de mezcla (Merge Sort)...\n")
